...
def prepare_inputfeatures(start, end, means_stds):
    ...
    for inputvar in FEATURES_FILES:
        ...
        raw_data = raw_data[:,:,:]
        raw_data = raw_data.reshape(raw_data.shape[0], NB_CELLS)
        nb_basins = raw_data.shape[0]/3650
        # TODO check this when adding new basins
        nb_basins = int(nb_basins)
        var_basins = {f"b{i}": raw_data[i*3650:3650*(i+1),:] for i in range(nb_basins)}
        data = np.array([])
        for basin in var_basins.keys():
            data = np.concatenate((data, var_basins[basin][start:end, :]), axis=0) if len(data)>0 else var_basins[basin][start:end, :]
        print(data.shape)
#        data = raw_data[start:end, :]
        ...
        lookback_arrays1 = [data[:, i-LOOKBACK:i] for i in range(LOOKBACK, end-start)]
        lookback_arrays2 = [data[:, i-LOOKBACK:i] for i in range(TRAINING_PERIOD+LOOKBACK, end*nb_basins-start*nb_basins)]
        lookback_arrays1.extend(lookback_arrays2)
        lookback_arrays = np.array(lookback_arrays1)
